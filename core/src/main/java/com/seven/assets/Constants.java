package com.seven.assets;

public class Constants {
    public static final String FLOOR = "floor.png";
    public static final String BOX = "box.png";
    public static final String TARGET = "target.png";
    public static final String WALL = "wall.png";
    public static final String SOLVED = "solved.png";
    public static final String PLAYER = "player.png";
    public static final float TILE_SIZE = 100f;
    public static final float PADDING = 4f;
    public static final float W1 = 1280f;
    public static final float H1 = 720f;
    public static final float W2 = 1920f;
    public static final float H2 = 1080f;
}
